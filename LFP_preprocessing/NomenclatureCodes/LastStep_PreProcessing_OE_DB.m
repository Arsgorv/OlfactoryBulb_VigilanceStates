...
%                     ExpeInfo);
        ...
        %% Define ndm-scripts folder
        % updated by AG on 15/04/2026 . ndm_dir is selected once and saved in a cfg file
        if ispc
            ndm_dir = GetNdmScriptsDir();
        end
        
        %% concatenate all the files
        %updated by AG on 15/04/2026 to be windows compatible ; 
        if length(ExpeInfo.PreProcessingInfo.FolderForConcatenation_Ephys) > 1
            if ispc
                % 1) Convert FinalFolder to WSL path (this is where the .dat/.xml live)
                drive_data = lower(FinalFolder(1));
                wsl_data_dir = strrep(FinalFolder, '\', '/');
                wsl_data_dir = regexprep(wsl_data_dir, '^[A-Za-z]:', ['/mnt/' drive_data]);

                % 2) Convert ndm_dir to WSL path
                drive_ndm = lower(ndm_dir(1));
                ndm_scripts_wsl = strrep(ndm_dir, '\', '/');
                ndm_scripts_wsl = regexprep(ndm_scripts_wsl, '^[A-Za-z]:', ['/mnt/' drive_ndm]);

                % 3) Build command: cd into data dir, call ndm_concatenate with ONLY BaseFileName                
                cmd = sprintf( ...
                    'wsl bash -lc ''export PATH="$PATH:%s"; cd "%s"; ndm_concatenate "%s"''', ...
                    ndm_scripts_wsl, wsl_data_dir, BaseFileName);
        ...
        %% make lfp
        %updated by AG on 29/11/2025 to be windows compatible
        if ispc
            % 1) Convert FinalFolder to WSL path (this is where the .dat/.xml live)
            drive_data = lower(FinalFolder(1));
            wsl_data_dir = strrep(FinalFolder, '\', '/');
            ...
            % 3) Build command: cd into data dir, call ndm_lfp with ONLY BaseFileName
            cmd = sprintf( ...
                'wsl bash -lc ''export PATH="$PATH:%s"; cd "%s"; ndm_lfp "%s"''', ...
                ndm_scripts_wsl, wsl_data_dir, BaseFileName);
