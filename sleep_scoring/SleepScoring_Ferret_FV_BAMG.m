...
%       - smoothwindow  Smoothing. Default = 3 seconds
...
%                          - Added Theta channel
%   Adapted for ferrets by Baptiste Maheo and Arsenii Goriachenkov -- 2022-2026
%      
...
for i = 1:2:length(varargin)
    ...
    switch(lower(varargin{i}))
        ...
        case 'full_ob'
            full_ob = varargin{i+1};
            if full_ob~=0 && full_ob ~=1
                error('Incorrect value for property ''full_ob''.');
            end
        case 'controlepoch'
...
% sleep scoring base only on OB oscillations
if ~exist('full_ob','var')
    full_ob=1;
end
...
disp('              Baptiste Maheo              \_   _/ ')
disp('              Arsenii Goriachenkov        `""^""` ')   
disp(' ')   
...
if doob
    ...
    % change frequency for [50 70]
        ...
        [SleepOB,SmoothGamma,Info_temp,microWakeEpochOB,microSleepEpochOB]= ...
            FindGammaEpoch_SleepScoring(Epoch, channel_bulb, minduration, 'foldername', foldername,...
            'smoothwindow', smootime, 'stimepoch', StimEpoch,'controlepoch',ControlEpoch,'frequency',[40 60]);
    end
    Info_OB=ConCatStruct(Info_OB,Info_temp);
    Info_OB.gamma_thresh = Info_temp.gamma_thresh ;
    Info_OB.gamma_minduration = Info_temp.gamma_minduration;
    Info_OB.gamma_OB_channel = Info_temp.gamma_OB_channel;
    clear Info_temp;
%     Sleep = SleepOB;
%     clear Sleep
    disp(' ')
...
if ~isempty(ImmobilityEpoch)
    ...
    Info_accelero.mov_threshold = Info_temp.mov_threshold;
    Info_accelero.mov_dropmerge = Info_temp.mov_dropmerge;
    Info_accelero.immob_dropmerge = Info_temp.immob_dropmerge;
    Info_accelero.manual_scoring = Info_temp.manual_scoring;
    clear Info_temp;
else
    is_accelero = false;
end
...
% REM using OB oscillations
if full_ob
    disp(' ')
    disp('------------------------------------------------------------')
    disp(' STEP 5 bis: DEFINING NREMEpoch and REMEpoch WITH OB GAMMA')
    disp('------------------------------------------------------------')
    disp(' ') 

    [GammaHighEpoch_OB, SmoothGamma_high, Info_temp] = ...
            FindREM_OBGamma_SleepScoring_Ferret_BM(SleepOB, Epoch, channel_bulb, minduration,...
            'foldername', foldername,'smoothwindow', smootime_sleep,'continuity',continuity,'frequency',[50 75]);
        
    Info_fullOB = ConCatStruct(Info_fullOB,Info_temp); 
    Info_fullOB.gamma_high_thresh = Info_temp.gamma_high_thresh;
    Info_fullOB.gamma_high_minduration = Info_temp.gamma_high_mindur;
    Info_fullOB.gamma_high_channel = Info_temp.gamma_high_channel;
    clear Info_temp;
...
%     Wake = Wake-TotalNoiseEpoch;
    ...
    try
        save('SleepScoring_OBGamma', 'ISEpoch', 'DeltaEpoch', 'REMEpoch','SWSEpoch','Wake', ...
            ...
            'Epoch','SubNoiseEpoch','TotalNoiseEpoch','-v7.3');
    catch
        save('SleepScoring_OBGamma', 'ISEpoch', 'DeltaEpoch', 'REMEpoch','SWSEpoch','Wake', ...
            'Sleep','SmoothGamma','ThetaEpoch','SmoothTheta', 'SmoothDelta_OB', 'Info',...
...
if full_ob
    ...
    Wake = or(Epoch-SleepOB,TotalNoiseEpoch);
    REMEpoch = and(SleepOB , GammaHighEpoch_OB);
    SWSEpoch = and(SleepOB , DeltaEpoch)-REMEpoch;
    ISEpoch = SleepOB-or(REMEpoch , SWSEpoch);
    [REMEpoch, SWSEpoch, ISEpoch, Wake , Sleep] = cleanSleepStates_BM(REMEpoch, SWSEpoch, ISEpoch, Wake, Epoch);
%     Wake = Wake-TotalNoiseEpoch;
    ...
    disp('           >>>  Saving full OB scoring stages  <<<')
    disp(' ')
    try
        save('SleepScoring_FullOB', 'ISEpoch', 'DeltaEpoch', 'REMEpoch','SWSEpoch','Wake', ...
            'Sleep','SmoothGamma','GammaHighEpoch_OB','SmoothGamma_high', 'SmoothDelta_OB', 'Info',...
            'Epoch','SubNoiseEpoch','TotalNoiseEpoch','-v7.3');
    catch
        save('SleepScoring_FullOB', 'ISEpoch', 'DeltaEpoch', 'REMEpoch','SWSEpoch','Wake', ...
            'Sleep','SmoothGamma','GammaHighEpoch_OB','SmoothGamma_high', 'SmoothDelta_OB', 'Info',...
...
%     OB Middle Spectrum
    ...
    if is_accelero
        ...
        Figure_SleepScoring_Accelero_Ferret(ratio_display_movement, foldername)
    end
